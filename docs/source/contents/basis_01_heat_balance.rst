...
:math:`k'_{o,eqv,j}` は :math:`0.7` である。

間仕切り等、裏面が室の場合、
:math:`k'_{s,ei,j,0}` ～ :math:`k'_{s,ei,j,J-1}` のどれかが :math:`1.0`,
:math:`k_{s,r,j,0}` ～ :math:`k_{s,r,j,I-1}` は :math:`0.0`、
...
:math:`\pmb{k}_{s,ei}` : :math:`k'_{s,ei,j,j^*}` を要素にもつ :math:`J \times J` の行列

:math:`\pmb{k}_{o,eqv}` : :math:`k_{EO,j}` を要素にもつ :math:`J \times 1` の縦行列
...
:math:`f_{mrt,i,j}` : 境界 |j| の室 |i| の微小球に対する形態係数
...
表面温度は応答係数方を用いて計算しているため、　:math:`\pmb{L}_{RS,n+1}` はステップ |n+1| における瞬時値である。
